"""Run from backend: .venv/bin/python -m app.ps1.scenario_a.cli --help."""
...
from app.ps1.scenario_a.search import SearchConfig, solve
...
def main(argv=None):
    ...
    try:
        ...
        if args.legacy:
            ...
            result = solve_other(instance, Scenario(args.scenario), replace(config, time_limit_seconds=remaining), stop.is_set, callback)
        if result.solution:
