"""Scenario A compatibility validator and independent geometry audit helpers."""
...
def validate_csvs(instance: Instance, files: dict[str, bytes], expected_cost: int | None = None) -> ValidationReport:
    # A must use the same closure gate as B/C and downloadable artifacts.
    from app.ps1.models import Scenario
    from app.ps1.validator import validate_exported_csvs
    result = validate_exported_csvs(instance, Scenario.A, files)
    if expected_cost is not None and result.feasible and abs(result.soft_scores["objective_score"] * 10 - expected_cost) > 1e-6:
        result.hard_violations.append({"rule": "objective", "severity": "hard", "detail": "Model and exported CSV objective differ."})
        result.feasible = False
        result.soft_scores.pop("objective_score", None)
        result.soft_scores.pop("formula_version", None)
    result.detail.update(policy=POLICY_VERSION, official_validator_available=False)
    return result
