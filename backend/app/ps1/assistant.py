...
def chat(job, scenario, solution, question, history, diff=None, pending_preview=None):
    ...
    scen_key = scenario.value if hasattr(scenario, "value") else (str(scenario) if scenario else "")
    if scen_key and scen_key not in ("A", "B", "C"):
        scen_key = ""
    ...
    def prepare_disruption(target_scenario: str = "", location_id: str = "", start_week: int = 0,
        ...
        target_scenario should be 'A', 'B' or 'C'. Defaults to active policy context if omitted.
        """
        nonlocal draft
        target_scenario = (target_scenario or scen_key).upper()
        run = job.scenarios.get(target_scenario)
    ...
    policy_ctx_label = f"Policy {scen_key}" if scen_key else "All Policies / None"
    user_query = f"[Active Policy Context: {policy_ctx_label}] {question}" if scen_key and not re.search(r"\b(?:scenario|policy)\s+[abc]\b", question, re.I) else question
