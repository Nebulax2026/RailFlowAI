...
export type EvidenceActivity = {
  ...
  accesses: { week: number; eclo: number; access_night: number }[];
...
export function Inspection({ activities, usage }: { activities: EvidenceActivity[]; usage: LocationUsage[] }) {
  ...
  return <div className="inspection">
    <section className="data-panel">
      ...
      <div className="activity-browser">
        ...
        {selected && <article className="activity-evidence" aria-label={`Evidence for ${selected.activity_id}`}>
          ...
          {table ? <div className="table-wrap"><table><thead><tr><th>Week</th><th>Local access night</th><th>Yield</th></tr></thead><tbody>{selected.accesses.map(a => <tr key={a.week}><td>{a.week}</td><td>{a.access_night}</td><td>{a.eclo ? "1.5 · ECLO" : "1 · Standard"}</td></tr>)}</tbody></table></div> : <ol className="access-timeline" aria-label="Scheduled access weeks">{selected.accesses.map(a => <li key={a.week} className={a.eclo ? "eclo" : ""}><strong>Week {a.week}</strong><span>Local night {a.access_night}</span><small>{a.eclo ? "ECLO · 1.5 units" : "Standard · 1 unit"}</small></li>)}</ol>}
          ...
          <p className="muted">Access-night numbers are local to the contract and week. Group labels are local to each location and week.</p>
    ...
    <section className="data-panel">
      <h3>Location and week inspection</h3>
      ...
      <p className="muted">Used slots include work possessions and protected slots under the documented conservative safety policy. Separate slots may occur in the same week.</p>
      <div className="table-wrap location-table"><table><thead><tr><th>Location</th><th>Week</th><th>Used / supply</th><th>Protection slots</th><th>Activities</th><th>Groups</th></tr></thead><tbody>{sites.map(u => <tr key={`${u.location_id}-${u.week}`}><td>{u.location_id}</td><td>{u.week}</td><td>{u.used} / {u.capacity}</td><td>{u.protection_possessions}</td><td>{u.activities.join(", ")}</td><td>{Object.entries(u.groups).map(([g,ids]) => `${g}: ${ids.join(", ")}`).join("; ") || "Protection only"}</td></tr>)}</tbody></table></div>
    </section>
  </div>;
}
