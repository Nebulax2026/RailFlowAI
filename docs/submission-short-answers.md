...
RailFlowAI transforms the eight PS1 demand-book CSVs into fully audited railway possession schedules for Scenarios A, B, and C. Powered by OR-Tools CP-SAT and an independent CSV validator, it strictly enforces network topology, weekly closures, and co-sharing rules, and re-validates every exported CSV before download. Works controllers get a command center that breaks each score into delay, extra-night and ECLO penalties; shows real-time supply headroom and high-strain sectors; audits contract milestones by priority tier; and compares delay slip across all three policies. Every activity is traceable to its scheduled nights, co-sharing partners and protection sectors. An evidence-grounded AI Assistant powered by Google Gemini answers plain-English schedule questions, explains why each scenario was scheduled that way, and runs live disruption re-planning—letting dispatchers test capacity cuts, re-optimize with minimal churn, and export submission-ready CSV packages.

**What tech stack was used to build this solution?**

The backend is Python 3.12 with FastAPI, Google OR-Tools CP-SAT for optimization, and Google Gemini via Vertex AI for grounded scheduling intelligence. The frontend uses Next.js 16, React 19, TypeScript, and Lucide icons for interactive analytics dashboards. Pytest covers parsing, topology, solver, CSV validation, and regression cases; TypeScript checks validate the frontend. Docker packages the application into a single service deployed on Google Cloud Run.

**What challenges did you face, and how did you overcome them?**

Ensuring official validator parity across tightly coupled railway constraints was our toughest hurdle. We explored diverse paradigms: greedy heuristics trapped early decisions into deadlocks, while Random and Adaptive LNS introduced search variance and struggled with complex weekly closure boundaries. Benchmarking proved that standard CP-SAT was decisively superior and more reliable. We pivoted to strengthening the core CP-SAT model directly: encoding official feedback into regression tests, enforcing strict closure exclusivity without heuristic shortcuts, and accurately penalizing contract completion delays. Paired with an independent CSV validator, this pure constraint programming approach eliminated illegal overlaps and produced validated schedules for all three scenarios.
