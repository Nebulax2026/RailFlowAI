...
2. The Assistant normalizes and validates location, weeks, capacity, and reason, then creates a ten-minute preview bound to the job, scenario, baseline revision, and parameters.
3. The preview changes no schedule. The controller explicitly selects **Run validated re-plan** or approves the displayed draft in chat (for example "approve" or "execute"). Changed parameters require a new preview and fresh approval.
