...
The 30 shared dataset inputs were regenerated with physical-night-consistent
witness schedules after the safety policy update. All 90 A/B/C witnesses pass
`readme-physical-night-v3`; A also passes its independent validator. The
reproducible audit and exact scores are in
`benchmarks/dataset-suite/current-validation.json`, generated by
`scripts/audit_dataset_suite.py`. Older benchmark averages used a previous
dataset revision and must not be presented as current-policy measurements.

Integration verification after the merge: 126 backend tests passed, including
the public search tests; frontend type generation, TypeScript and production
build passed.
